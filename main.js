...
// Define a function to remove the watermark text
function removeWatermarkText(node) {
  const watermarkText = "Ninja Slider trial version";
  if (node.textContent.includes(watermarkText)) {
    node.textContent = node.textContent.replace(watermarkText, "");
  }
}
...
// Start observing changes to the entire DOM
